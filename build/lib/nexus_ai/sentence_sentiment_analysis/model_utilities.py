...
from nexus_ai.sentence_sentiment_analysis.preprocessing import tokenize_data, pad_features, google_clean_reviews
...
def predict(data_path=None, dataframe=None, reviews=None, labels=None, column=None, gpu=False, seq_length=20, batch_size=1000,
    ...
    if not net:
        ...
        net = model.SentimentRNN(vocab_size = vocab_size, output_size = checkpoint['output_size'],
...
def train(net, train_loader, valid_loader, optimizer, vocab_to_int, batch_size, seq_length, epochs, print_every,
          train_on_gpu, multi_class=False, save_dic='test_01.pth', l1_lambda=None):
    ...
    for e in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            if l1_lambda:
                loss = criterion(output, labels.long()) + l1_penalty(net.parameters(), l1_lambda)
            else:
            ...
            if torch.isnan(inputs).any() or torch.isinf(inputs).any():
                print('invalid input detected at epoch ', e)
            ...
            if counter % print_every == 0:
                ...
                if valid_loss <= valid_loss_min:
                    ...
                    if isinstance(net, multi_class_model.ar_word2vec_RNN):
                        word2vec = True
                    else:
                        word2vec = False
...
def test(test_loader, train_on_gpu, batch_size, multi_class=False, net=None, model_path=None):
    ...
    if not net:
        ...
        vocab_to_int = checkpoint['vocab_to_int']
        if not checkpoint['word2vec']:
            vocab_size = len(vocab_to_int)+1
        else:
            vocab_size = len(vocab_to_int)
        if not checkpoint['multi_class']:
            ...
            net = multi_class_model.ar_word2vec_RNN(
                vocab_size = vocab_size, output_size = checkpoint['output_size'],
                embedding_dim = checkpoint['embedding_dim'], hidden_dim = checkpoint['hidden_dim'],
                n_layers = checkpoint['n_layers'], train_on_gpu=train_on_gpu,
                drop_prob = 0, bidirectional = checkpoint['bidirectional']
